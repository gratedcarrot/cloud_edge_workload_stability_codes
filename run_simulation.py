...
SCENARIO_ORDER = ['low_load', 'medium_load', 'high_load', 'burst_load', 'extended_workload']
...
def aggregate_with_ci(summary_df: pd.DataFrame, group_cols: List[str], metric_cols: Optional[List[str]]=None) -> pd.DataFrame:
    if metric_cols is None:
        metric_cols = ['throughput_tasks_per_s', 'avg_response_time_ms', 'avg_waiting_time_ms', 'edge_utilization', 'cloud_utilization', 'avg_edge_queue_length', 'avg_cloud_queue_length', 'blocking_probability', 'cloud_offloading_ratio', 'sla_violation_rate']
    rows = []
    ...
    if 'mode' in out.columns:
        out['mode'] = pd.Categorical(out['mode'], categories=PROCESSING_MODES, ordered=True)
...
def run_sensitivity_analysis(tasks_df: pd.DataFrame, params_df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    ...
    for scenario in scenarios:
        ...
        for run_id in sorted(scenario_df['run_id'].unique()):
            ...
            for edge_servers in EDGE_SERVER_VALUES:
                ...
                _, summary = simulate_hybrid(subset_base, params, threshold_ms=HYBRID_EDGE_DELAY_THRESHOLD_MS)
            ...
            for multiplier in NETWORK_DELAY_MULTIPLIERS:
                ...
                _, summary = simulate_hybrid(subset, base_params, threshold_ms=HYBRID_EDGE_DELAY_THRESHOLD_MS)
...
def plot_metric_grouped(agg_df: pd.DataFrame, metric_mean_col: str, ylabel: str, output_path: Path, metric_ci_col: Optional[str]=None) -> None:
    ...
    plot_df = plot_df.sort_values(['scenario', 'mode'])
    scenarios = [s for s in SCENARIO_ORDER if s in set(plot_df['scenario'].astype(str))]
    modes = [m for m in PROCESSING_MODES if m in set(plot_df['mode'].astype(str))]
...
def write_method_note(output_dir: Path, args: argparse.Namespace) -> None:
    note = '\nSimulation Method Note\n==============================\n\nThe simulation engine evaluates edge-only, cloud-only, and hybrid\ncloud-edge processing using task-level workload traces. Each scenario is\nrepeated across independent runs generated with fixed seeds. The same trace is\nused for all processing modes within each run, ensuring fair comparison.\n\nThe simulator records task-level completion status, processing location,\nwaiting time, response time, queue estimates, SLA violation status, and cloud\ncommunication delay. Finite waiting-buffer capacity is enforced for both edge\nand cloud layers. Summary metrics include average response time, average\nwaiting time, throughput, utilization, average queue length, blocking\nprobability, cloud offloading ratio, and SLA violation rate.\n\nScenario/mode results are reported with sample mean, standard deviation, and\n95% confidence intervals. Statistical tests are generated when scipy is\navailable. Optional sensitivity analysis evaluates threshold, edge capacity,\ncloud capacity, edge queue capacity, and network delay under high-load and\nburst-load conditions.\n'.strip()
    (output_dir / 'simulation_method_note.txt').write_text(note, encoding='utf-8')
    config = {'input_dir': str(args.input_dir), 'output_dir': str(args.output_dir), 'baseline_hybrid_threshold_ms': HYBRID_EDGE_DELAY_THRESHOLD_MS, 'service_time': {'min_service_time_ms': MIN_SERVICE_TIME_MS, 'nominal_cpu_demand_mi': NOMINAL_CPU_DEMAND_MI}, 'cloud_communication': {'edge_cloud_bandwidth_mbps': EDGE_CLOUD_BANDWIDTH_MBPS, 'result_return_delay_factor': RESULT_RETURN_DELAY_FACTOR, 'min_return_delay_ms': MIN_RETURN_DELAY_MS}, 'sensitivity_enabled': bool(args.sensitivity), 'sensitivity_scenarios': SENSITIVITY_SCENARIOS, 'threshold_values_ms': THRESHOLD_VALUES_MS, 'edge_server_values': EDGE_SERVER_VALUES, 'cloud_server_values': CLOUD_SERVER_VALUES, 'edge_queue_values': EDGE_QUEUE_VALUES, 'network_delay_multipliers': NETWORK_DELAY_MULTIPLIERS, 'scipy_available_for_stats': scipy_stats is not None}
